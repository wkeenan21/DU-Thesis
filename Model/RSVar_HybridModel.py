...
class HybridModel:
    ...
    def TrainModelProgressive(self, hours_used=480, traintype=0, recentsampleweight=1, waitround=3):
        ...
        if self.leveloption == 1:
            ...
            if valid_cnt > 0:
                self.gnn_lstm_model_rs.fit(
                    x=[neighborinput_data_l1_feature[valid_day_list],
                       neighborinput_data_l1_dis[valid_day_list],
                       neighborinput_data_l2_feature[valid_day_list],neighborinput_data_l2_feature_Var[valid_day_list],
                       neighborinput_data_l2_dis[valid_day_list], selfinputput_data[valid_day_list],
                       selfinput_lstm_data[valid_day_list],
                       selfinput_lstm_rs_data[valid_day_list]],
                    y=selfoutput_data[valid_day_list].reshape(valid_cnt, 1),
                    batch_size=self.batch_size,
                    epochs=(self.epo*2), verbose=0)
        else:
            self.gnn_lstm_model.fit(
            ...
            if valid_cnt > 0:
                self.gnn_lstm_model_rs.fit(
                    x=[neighborinput_data_l2_feature[valid_day_list],neighborinput_data_l2_feature_Var[valid_day_list],
                       neighborinput_data_l2_dis[valid_day_list], selfinputput_data[valid_day_list],
                       selfinput_lstm_data[valid_day_list],
                       selfinput_lstm_rs_data[valid_day_list]],
                    y=selfoutput_data[valid_day_list].reshape(valid_cnt, 1),
        ...
        for i_round in range(total_rounds - trainrounds - 1):
            ...
            if self.leveloption == 1:
                ...
                if valid_cnt > 0:
                    selfoutput_data_pred_model_rs[[valid_day_list]] = self.gnn_lstm_model_rs.predict(
                        [neighborinput_data_l1_pred_feature[valid_day_list],
                         neighborinput_data_l1_pred_dis[valid_day_list],
                         neighborinput_data_l2_pred_feature[valid_day_list],neighborinput_data_l2_pred_feature_Var[valid_day_list],
                         neighborinput_data_l2_pred_dis[valid_day_list],
                         selfinputput_data_pred[valid_day_list], selfinput_lstm_data_pred[valid_day_list],
                         selfinput_lstm_rs_data_pred[valid_day_list]])
                    ...
                    for i_fit_round in range(recentsampleweight):
                        ...
                        if valid_cnt > 0:
                            self.gnn_lstm_model_rs.fit(x=[
                                ...
                                selfinputput_data_pred[valid_day_list], selfinput_lstm_data_pred[valid_day_list],
                                selfinput_lstm_rs_data_pred[valid_day_list], ],
                                y=selfoutput_data_pred[valid_day_list].reshape(valid_cnt, 1),

                                batch_size=self.batch_size,
                                epochs=self.epo, verbose=0)
                else:
                    for i_fit_round in range(recentsampleweight):
                        self.gnn_lstm_model.fit(
                        ...
                        if valid_cnt > 0:
                            ...
                            self.gnn_lstm_model_rs.fit(x=[neighborinput_data_l2_pred_feature[valid_day_list],neighborinput_data_l2_pred_feature_Var[valid_day_list],
                                                          neighborinput_data_l2_pred_dis[valid_day_list],
                                                          selfinputput_data_pred[valid_day_list],
                                                          selfinput_lstm_data_pred[valid_day_list],
                                                          selfinput_lstm_rs_data_pred[valid_day_list], ],
                                                       y=selfoutput_data_pred[valid_day_list].reshape(valid_cnt, 1),
    ...
    def CreateLocalModelTimeSeriesRS(self, featuredim=1, useVar=True):
        ##self node has the complete information
        nodeself = Input(shape=(self.inputdim_self))
        nodeprops_self = Dense(self.gcn_dim, activation='tanh')(nodeself)
        nodeprops_self = Dropout(self.dropoutrate)(nodeprops_self)

        nodeself_weather_lstm = Input(shape=(self.timesize_l2, len(self.fields_weather)))
        nodeself_weather_lstm_scale = Dense(self.embed_dim)(nodeself_weather_lstm)
        nodeself_weather_lstm_scale = Dropout(self.dropoutrate)(nodeself_weather_lstm_scale)
        nodeself_weather_lstm_out = LSTM(self.embed_dim)(nodeself_weather_lstm_scale)
        nodeself_weather_lstm_out = Dropout(self.dropoutrate)(nodeself_weather_lstm_out)

        ##todo: rs data
        nodeself_rs_lstm = Input(shape=(self.timesize_l1, 1))
        nodeself_rs_lstm_scale = Dense(self.embed_dim)(nodeself_rs_lstm)
        nodeself_rs_lstm_scale = Dropout(self.dropoutrate)(nodeself_rs_lstm_scale)
        nodeself_rs_lstm_out = LSTM(self.embed_dim)(nodeself_rs_lstm_scale)
        nodeself_rs_lstm_out = Dropout(self.dropoutrate)(nodeself_rs_lstm_out)

        # kernel_regularizer = regularizers.l1_l2(l1=1e-5, l2=1e-4),
        # bias_regularizer = regularizers.l2(1e-4),
        # activity_regularizer = regularizers.l2(1e-5)
        ##Todo: access this from the trainned model
        if self.leveloption == 1:
            neighbor_time_list_l1_feature = Input(shape=(self.timesize_l1, self.neigbhorsize_l1, featuredim))
            neighbor_time_list_l1_dis = Input(shape=(self.timesize_l1, self.neigbhorsize_l1, 1))
        ...
        ##todo: better way to implement the loop
        if self.leveloption == 1:
            aq_l1 = self.CreateLocalModelAggrDynamic(neighbor_time_list_l1_feature, neighbor_time_list_l1_dis,
                                                     self.neigbhorsize_l1, self.timesize_l1, )
            aq_l1 = Dropout(self.dropoutrate)(aq_l1)
        ...
        aq_l2_Var = self.CreateLocalModelAggrDynamic(neighbor_time_list_l2_feature_Var, neighbor_time_list_l2_dis,
                                                        self.neigbhorsize_l2, self.timesize_l2, )
        aq_l2_Var = Dropout(self.dropoutrate)(aq_l2_Var)
        ...
        if self.leveloption == 1:
            ...
            aq_combine = Concatenate()([aq_l1, aq_l2, aq_l2_Var, nodeself_weather_lstm_out, nodeself_rs_lstm_out,nodeprops_self])

        else:

            aq_combine = Concatenate()([aq_l2, aq_l2_Var, nodeself_weather_lstm_out, nodeself_rs_lstm_out, nodeprops_self])

        aq_final = Dense(self.outputdim, activation='tanh')(aq_combine)
        aq_final = Dropout(self.dropoutrate)(aq_final)

        if self.leveloption == 1:
            gnn_lstm_model = Model(inputs=[neighbor_time_list_l1_feature, neighbor_time_list_l1_dis,
                                           neighbor_time_list_l2_feature, neighbor_time_list_l2_feature_Var,
                                           neighbor_time_list_l2_dis,
                                           nodeself, nodeself_weather_lstm,nodeself_rs_lstm], outputs=aq_final)
        ...
        self.gnn_lstm_model_rs = gnn_lstm_model
    ...
    def CreateLocalModelTimeSeries(self, featuredim=1, useVar=True):
        ...
        ##todo: learn weights or attention scores--self attention? simple first
        if self.leveloption == 1:
            aq_combine = Concatenate()([aq_l1, aq_l2, aq_l2_Var, nodeself_weather_lstm_out, nodeprops_self])
        else:

            aq_combine = Concatenate()([aq_l2, aq_l2_Var, nodeself_weather_lstm_out, nodeprops_self])
    ...
    def CreateLocalModelAggrDynamic(self, neighbor_list, neighbor_dis_list, neighborhoodsize, timedimension=1,
        ...
        neighbor_scale = Dropout(self.dropoutrate)(neighbor_scale)
        ...
        if self.useConv == 3:
            ...
            avg_conv2dlstm = ConvLSTM2D(filters=filter_cnt, kernel_size=(neighborhoodsize, 1),
                                        strides=(neighborhoodsize, 1),
                                        padding='valid', )(neigbhor_edge_weight_reshape)
            avg_conv2dlstm = Dropout(self.dropoutrate)(avg_conv2dlstm)
            ##or permute -->dense
            avg_neighbor_flat = Flatten()(avg_conv2dlstm)
            # avg_neighbor_flat = Dropout(self.dropoutrate)(avg_neighbor_flat)
            avg_neighbor = Dense(self.gcn_dim)(avg_neighbor_flat)
            aq_neighbor = Dropout(self.dropoutrate)(avg_neighbor)
            return aq_neighbor
        else:
            if self.useConv == 0:
                if useMax:
                    avg_layer = MaxPooling2D(pool_size=(1, neighborhoodsize))(neigbhor_edge_weight)
                else:
                    avg_layer = AveragePooling2D(pool_size=(1, neighborhoodsize))(neigbhor_edge_weight)
            elif self.useConv == 1:
...
class NegativePowerLayer(tf.keras.layers.Layer):
    def __init__(self):
        super(NegativePowerLayer, self).__init__()
        self.supports_masking = True

    def call(self, inputs):
        # boolean_mask = math_ops.not_equal(mask_input, self.mask_value)
        return K.exp(K.pow(inputs, 2) * -1)
        # return K.ones_like(inputs * inputs * -1)
